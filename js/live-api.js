  ...
  async function refreshDashboard() {
    ...
    try {
      ...
      $("#content").innerHTML = `<div class="home-layout">
        <section class="today-hero"><div><p class="eyebrow">VOTRE JOURNÉE, EN CLAIR</p><h2>Aujourd’hui</h2><p>${failed ? "Certaines informations sont indisponibles. Actualisez pour réessayer." : count ? `${count} priorité${count > 1 ? "s" : ""} à regarder pour avancer aujourd’hui.` : "Vos priorités sont à jour. Vous pouvez préparer la suite."}</p><div class="today-date">${esc(today.toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" }))}</div></div><button data-command="Organise ma journée à partir des tâches, relances et validations en attente.">Organiser ma journée</button></section>
        ...
        <section class="workspace-kpis" aria-label="Votre activité"><div class="workspace-kpi"><strong>${number(s?.clients)}</strong><span>Clients actifs</span></div><div class="workspace-kpi"><strong>${data.deals ? number(deals.filter((d) => !["won", "lost"].includes(d.stage)).length) : "—"}</strong><span>Prospects à suivre · vue chargée</span></div><div class="workspace-kpi"><strong>${number(s?.projects)}</strong><span>Projets en cours</span></div><div class="workspace-kpi"><strong>${number(s?.tasks)}</strong><span>Tâches à terminer</span></div></section>
