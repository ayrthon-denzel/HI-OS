...
function parseJson(text){
  ...
  const a1=stripped.indexOf('['), a2=stripped.lastIndexOf(']');
  if(a1>=0 && a2>a1){ try{return JSON.parse(stripped.slice(a1,a2+1));}catch{} }
...
async function responses({input,instructions,webSearch=false}){
  ...
  const body={model:MODEL,input,instructions,store:false};
...
async function findJobs(profile){
  ...
  const parsed=parseJson(out.text); const items=Array.isArray(parsed)?parsed:(parsed.items||[]);
  return items.slice(0,20).filter(x=>x && x.company && x.role_title && x.url).map((x,i)=>({
    source:String(x.source||'web').slice(0,80),
    external_id:String(x.external_id||x.url||`web-${Date.now()}-${i}`).slice(0,300),
    company:String(x.company).slice(0,180),role_title:String(x.role_title).slice(0,180),location:String(x.location||'').slice(0,180),
    url:String(x.url).slice(0,1500),posted_at:x.posted_at||null,score:Math.max(0,Math.min(100,Number(x.score)||0)),
    rationale:String(x.rationale||'').slice(0,1000)
  }));
}
...
async function tailorApplication({cvText,opportunity,profile}){
  ...
  const out=await responses({input:prompt,instructions:'Priorité absolue à la vérité factuelle du CV source. Aucune invention. Style professionnel, ATS-friendly et concis.'});
  const p=parseJson(out.text);
...
async function orchestrate(command){
  const prompt=`Commande CEO: ${String(command||'').slice(0,4000)}\nRetourne UNIQUEMENT un objet JSON avec: summary, route (tableau d'agents), actions (tableau d'objets {agent,action,permission}), requires_human (booléen). Les permissions possibles sont A0,A1,A2,A3. Toute dépense, contrat, suppression irréversible, changement de droits admin ou publication sensible doit être A3.`;
  const out=await responses({input:prompt,instructions:'Tu es HI Orchestrator. Respecte le principe du moindre privilège et la politique A0-A3.'});
