...
express.application.listen=function(...args){
  if(!installed){
    ...
    if(DATABASE_URL){
      const pool=new Pool({connectionString:DATABASE_URL,ssl:{rejectUnauthorized:false},max:3,idleTimeoutMillis:30000,connectionTimeoutMillis:8000});
      ...
      let ready=pool.query(schema).then(()=>console.log('HI OS workspace schema ready')).catch(e=>{console.error('workspace_schema',e.message);throw e;});
      ...
      this.use('/api/workspace',auth,workspaceRoutes({pool,tenantFor,audit,clean}));
      console.log('HI OS workspace routes mounted');
    }
  }
